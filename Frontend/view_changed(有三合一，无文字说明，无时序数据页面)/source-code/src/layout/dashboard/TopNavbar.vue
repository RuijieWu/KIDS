<template>
  <nav class="navbar navbar-expand-lg navbar-light">
    <div class="container-fluid">
      ...
      <!--  <el-tooltip class="item custom-tooltip" effect="dark" :content="pageDescription" placement="bottom">
...
export default {
  data() {
    return {
      ...
      pageDescriptions: {
        "Dashboard": "  安全态势感知页面展示了引擎输出数据的统计结果，其中四个状态卡片分别代表可疑/危险攻击方数，可疑/危险被攻击方数和可疑/危险行为数。状态卡片的下方使用chartlist中的折线图统计了七日内的可疑/危险行为数量和今日以及昨日不同时间段内的行为数量，使用了echart中的饼图对行为类型，攻击方/被攻击方类型进行了统计与展示，堆叠式柱状图统计了最频繁出现的6个攻击方/被攻击方及其相关的行为。最后使用自定义表格子组件展示了近120条危险/可疑行为的详细信息，上述总体统计数据的展示有利于用户对安全态势有一个总体的认知，便于用户后续使用",
