<template>
  <div>
    ...
    <el-menu
      ...
      active-text-color="#ffd04b">
        ...
        <el-menu-item index="rules">规则预检</el-menu-item>
     
    
        <el-menu-item index="analysis">AI模型检测</el-menu-item>
      
      
        <el-menu-item index="events">事件输出</el-menu-item>
    ...
    <div class="container mt-4">
      <!-- 分析页面 -->
      <div v-if="currentWebPage !='analysis' && currentWebPage !='rules' &&currentWebPage !='events'">
      <el-steps :active="4">
  <el-step title="第一层过滤" description="规则预检"></el-step>
  <el-step title="第二层过滤" description="AI模型检测"></el-step>
  <el-step title="分析结果输出" description="安全事件输出"></el-step>
</el-steps>
</div>
      <div v-if="currentWebPage === 'analysis'">
        <div class="main-container">
          <div class="main-container">
            <span style="font-size: 18px;">"KIDS系统对于数据的第二层检测，使用KIDS引擎分析规则预检后的数据，将时序数据分化为时间窗口，
              每个异常时间窗口对应一个安全事件，引擎输出每个安全事件对应的溯源图"</span>
      <div class="top-row">
  ...
  <div class="row mt-0">
  <el-button @click="showModelConfig"  class="mt-0 ml-4">修改模型配置</el-button>
</div>
       </div> 
...
/>
      ...
      <el-button @click="toggleActionView" type="primary">
    ...
    <div class="source-map" v-else>
      ...
      <!-- 规则页面 -->
      <div v-if="currentWebPage === 'rules'">
          <div class="main-container">
  <div class="d-flex justify-content-end mb-3">
    <span class="mr-5" style="font-size: 18px;">"KIDS系统对于数据的第一层过滤，此页面允许用户自定义黑白名单规则，
      实现对数据的快速处理，处理后的数据插入数据库，使用AI进行第二层检测"</span>
  ...
  <div class="rule-engine" v-if="!showTable">
    ...
    <el-dialog :visible.sync="dialogVisible" title="选择文件并上传" width="50%" :modal-append-to-body="false">
      ...
      <!-- 事件页面 -->
      <div v-if="currentWebPage === 'events'">
          <div class="row">
      <div class="col-12">
        <span style="font-size: 18px;">"此页面针对每个安全事件提供用户可查看的溯源图和统计数据，生成交互式安全报告并且处理其中间过程得到关键路径报告，
          提供模板化的安全建议以及使用大语言模型生成的安全建议"</span>
        <div class="table-header">
          <div class="search-bar">
            <input v-model="searchText" type="text" placeholder="搜索..." class="form-control" />
            <p-button class="search-btn ti-search"></p-button>
  ...
  filteredRules() {
    ...
    filteredSources() {
        ...
        return this.MapTable.data.filter(source => 
  ...
  currentWebPage(newPage, oldPage) {
      // 当页面切换时，调用对应的初始化方法
      switch(newPage) {
        case 'analysis':
          this.initAnalysisPage();
...
addEmptyRule() {
  ...
  try {
    ...
    if (response.data && response.data.data) {
      this.MapTable.data = response.data.data.map((item, index) => {
        // 从文件名中提取时间段
      ...
      // 更新总数
      this.MapTable.total = response.data.total;
    }
  } catch (error) {
