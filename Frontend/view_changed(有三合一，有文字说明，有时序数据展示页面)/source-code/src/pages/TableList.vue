  ...
  <div class="row">
    ...
    <div class="table-selector">
      ...
      <card :title="CurrentTable.title" :subTitle="CurrentTable.subTitle" v-if="CurrentTable">
        <div slot="raw-content" class="table" v-loading="CurrentTable.loading" element-loading-text="加载中...">
          <paper-table 
...
watch: {
  ...
  methods:{
    ...
    async fetchAttackerData(){
      this.AttackerTable.loading = true;
      ...
      const response_kairos = await axios.get('http://43.138.200.89:8080/kairos/subjects',{
        ...
        this.$set(this.AttackerTable, 'data', [...this.AttackerTable.data, ...newData]);
        this.AttackerTable.loading = false;
        this.dataUpdateTrigger += 1;
    },
    async fetchAttackedData(){
      this.AttackedTable.loading = true;
      function formatTime(timestamp) {
        const date = new Date(Number(BigInt(timestamp) / 1000000n));
        const year = date.getFullYear();
        const month = String(date.getMonth() + 1).padStart(2, '0');
      ...
      const response_kairos = await axios.get('http://43.138.200.89:8080/kairos/objects',{
        ...
        this.$set(this.AttackedTable, 'data', [...this.AttackedTable.data, ...newData_object])
        this.AttackedTable.loading = false;
        this.dataUpdateTrigger += 1;
        this.updateAllFilteredData();
    },
    async  fetchAlertData() {
      this.AlterTable.loading = true;
      this.DangerTable.loading = true;
      function formatTime(timestamp) {
        const date = new Date(Number(BigInt(timestamp) / 1000000n));
        const year = date.getFullYear();
        const month = String(date.getMonth() + 1).padStart(2, '0');
       ...
       /* const response = await axios.get('http://43.138.200.89:8080/alarm/message/list', {
        ...
        this.$set(this.AlterTable, 'data', [...this.AlterTable.data, ...newAlterData])
        this.$set(this.DangerTable, 'data', [...this.DangerTable.data, ...newDangerData])
        this.AlterTable.loading = false;
        this.DangerTable.loading = false;
        this.dataUpdateTrigger += 1;
      },
